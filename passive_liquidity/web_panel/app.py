...
def _orders_summary_text_from_rows(rows: list[dict[str, Any]]) -> str:
    lines = [
        "Ordens Abertas em Tempo Real",
        f"Total de ordens: {len(rows)}",
    ]
    if not rows:
        lines.append("(Nenhuma ordem aberta no momento)")
        return "\n".join(lines)
    for idx, r in enumerate(rows, start=1):
        oid = str(r.get("order_id") or "")
        lines.append(f"{idx}) Mercado: {str(r.get('market_title') or '(Mercado Desconhecido)')}")
        lines.append(f"   order_id={oid}")
        lines.append(
            "   lado={side}  preço={price}  volume={size}".format(
...
def create_app() -> Flask:
        ...
        if not expect:
            return (
                render_template(
                    ...
                    message="Por favor, defina WEB_PANEL_TOKEN (senha de login) no seu arquivo .env.",
                ),
                503,
            )
    ...
    def api_order_custom_rule() -> Any:
        ...
        if not token_id or side not in ("BUY", "SELL"):
            return jsonify({"error": "token_id e side (BUY/SELL) são obrigatórios"}), 400
        ctx = get_ctx()
    ...
    def cancel_order() -> Any:
        ...
        if not oid:
            flash("order_id ausente", "error")
            return redirect(url_for("orders_page"))
        ctx = get_ctx()
        try:
            ctx.client.cancel(oid)
            _cache_invalidate()
            flash(f"Cancelamento enviado: {oid[:24]}…", "ok")
        except Exception as e:
            LOG.warning("web cancel failed: %s", e)
            flash(f"Falha ao cancelar: {e}", "error")
        return redirect(url_for("orders_page"))
    ...
    def cancel_all() -> Any:
        ...
        try:
            ...
            flash(f"Falha ao buscar ordens: {e}", "error")
            return redirect(url_for("orders_page"))
        total = 0
        failed = 0
        for o in orders:
            ...
            try:
                ...
                failed += 1
        if total == 0:
            flash("Nenhuma ordem aberta.", "ok")
        elif failed == 0:
            _cache_invalidate()
            flash(f"Cancelamento de todas as {total} ordens enviado.", "ok")
        else:
            flash(f"Falha parcial: Sucesso {total - failed}/{total}.", "error")
        return redirect(url_for("orders_page"))
    ...
    def rules_add() -> Any:
        ...
        if not token_id or side not in ("BUY", "SELL"):
            flash("token_id e side (BUY/SELL) são obrigatórios", "error")
        ...
        try:
            ...
            flash(f"Parâmetros inválidos: {e}", "error")
            return redirect(back)
        ctx.rules_store.set_rule(token_id, side, rule)
        _cache_invalidate()
        flash(f"Regra salva: {stable_rule_key(token_id, side)}", "ok")
        return redirect(back)
    ...
    def rules_delete() -> Any:
        ...
        side = (request.form.get("side") or "").strip().upper()
        if not token_id or side not in ("BUY", "SELL"):
            flash("token_id e side são obrigatórios", "error")
            return redirect(back)
        if ctx.rules_store.clear_rule(token_id, side):
            _cache_invalidate()
            flash("Regra excluída", "ok")
        else:
