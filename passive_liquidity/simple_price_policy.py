...
def format_eligible_band_depth_summary_zh(
    ...
    *,
    ...
    """Linhas extras para o resumo periódico de banda no Telegram (Português)."""
    lines = [
        f"  Estatísticas de profundidade no intervalo [{stats.scan_lo:.4f},{stats.scan_hi:.4f}] regime={stats.tick_regime}",
    ]
    if not stats.price_sizes:
        lines.append("  Profundidade por preço: Nenhuma (sem ordens neste lado)")
    else:
        shown = stats.price_sizes[:max_levels]
        seg = " ".join(f"{p:.4f}:{s:g}" for p, s in shown)
        if len(stats.price_sizes) > max_levels:
            seg += f" …total {len(stats.price_sizes)} níveis"
        lines.append(
            f"  Profundidade por preço: {seg} | Total no intervalo {stats.total_in_band:g}"
        )
    if stats.total_in_band <= 1e-12:
        lines.append("  Profundidade mais próxima do mid que esta ordem: — (Total no intervalo é 0)")
    elif stats.pct_closer_of_band is not None:
        lines.append(
            f"  Profundidade mais próxima do mid que esta ordem: {stats.closer_to_mid_than_order:g} "
            f"({stats.pct_closer_of_band:.1f}% do total no intervalo)"
        )
    else:
        lines.append(
            f"  Profundidade mais próxima do mid que esta ordem: {stats.closer_to_mid_than_order:g}"
