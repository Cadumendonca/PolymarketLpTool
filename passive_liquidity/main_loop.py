...
def main() -> None:
    ...
    if telegram.enabled:
        ...
        if not config.alert_monitoring_enabled and config.telegram_notify_fill:
            LOG.warning(
                "PASSIVE_ALERT_MONITORING=false desativa apenas alertas de fita/profundidade; "
                "notificações de execução de ordem continuam ATIVAS (PASSIVE_TELEGRAM_NOTIFY_FILL). "
                "Defina PASSIVE_TELEGRAM_NOTIFY_FILL=false para parar as mensagens de execução."
            )
    else:
        LOG.info("Telegram notifications disabled or misconfigured")

    print(
        "Monitoramento de Whitelist (veja abaixo). Se o desfecho (outcome) já tiver uma posição (estoque não zero), o token será ignorado completamente (sem ajuste, sem cancelamento).\n"
        "· Se PASSIVE_TOKEN_WHITELIST estiver definido: apenas as variáveis de ambiente serão usadas.\n"
        "· Caso contrário: os token_ids serão extraídos das ordens abertas atuais e atualizados a cada 120s (PASSIVE_WHITELIST_REFRESH_SEC por padrão).\n"
        "O ajuste de preço segue apenas regras simplificadas; o bot não criará novas ordens.\n",
        flush=True,
    )
    ...
    print(
        f"【Whitelist Iniciada】Fonte={wl_source} {seed_note} Tokens Únicos={len(frozen_whitelist)} "
        f"Limite de Estoque={config.inventory_manual_threshold}{dyn_note}\n",
        flush=True,
    )
    if frozen_whitelist:
        for tid in sorted(frozen_whitelist):
        ...
        if not config.token_whitelist and config.whitelist_refresh_interval_sec > 0:
            print("  (Vazio) Tokens serão adicionados automaticamente quando houver ordens abertas.\n", flush=True)
        else:
            print("  (Vazio) Por favor, abra ordens ou configure PASSIVE_TOKEN_WHITELIST.\n", flush=True)
    ...
    if snap0:
        ...
        if polygon_summary is not None and polygon_summary.approximate:
            ...
            extra_note = (
                "Nota: Não foi possível obter o valor de depósito de referência automaticamente. 'Total da Conta' é a soma do Colateral CLOB + Valor de Mercado das Posições, "
                "que não é o mesmo que depósitos totais históricos. Configure TELEGRAM_TOTAL_DEPOSITED_USDC no seu .env "
                "ou configure uma POLYGONSCAN_API_KEY."
            )
        elif "Bridge API" in deposit_source_zh:
            extra_note = "Nota: Referência de depósito obtida via Bridge API da Polymarket (depósitos de USDC na Polygon)."
        elif "referência temporária" in deposit_source_zh:
            extra_note = (
                "Nota: PASSIVE_USE_STARTUP_TOTAL_AS_DEPOSIT_REF está ativado; "
                "esta referência pode não ser o valor real de depósitos, usada apenas para cálculo temporário de PnL."
            )
        if telegram.enabled:
        ...
        try:
            ...
            if (
                ...
                if uc != prev_user_ws_connected:
                    if uc:
                        telegram.notify_ws_transport_zh(
                            title_zh="WebSocket: Canal do Usuário Conectado",
                            lines=[
                                "Notificações: Ordens/Execuções deste usuário (monitoramento); "
                                "Cancelamentos e ajustes continuam sendo feitos via REST pelo loop principal."
                            ],
                        ...
                        telegram.notify_ws_transport_zh(
                            title_zh="WebSocket: Canal do Usuário Desconectado",
                            lines=[
                                "Usando fallback REST para detecção de execuções; loop principal continua normalmente.",
                                f"último_erro={dbg.get('user_error') or '—'}",
                            ],
                ...
                if mc != prev_market_ws_connected:
                    if mc:
                        telegram.notify_ws_transport_zh(
                            title_zh="WebSocket: Canal de Mercado Conectado",
                            lines=[
                                "Notificações: Atividade do Orderbook/Trades (monitoramento); "
                                "Fallback automático para REST em caso de atraso ou desconexão."
                            ],
                        ...
                        dbg = ws_hub.connection_debug()
                        telegram.notify_ws_transport_zh(
                            title_zh="WebSocket: Canal de Mercado Desconectado",
                            lines=[
                                "Monitoramento de profundidade e atividade de fita em fallback REST.",
                                f"último_erro={dbg.get('market_error') or '—'}",
                            ],
            ...
            if not orders:
                ...
                for row in cycle_rows:
                    ...
                    def _on_replace_post_retry(attempt: int, err: str) -> None:
                        ...
                        lines = [
                            f"ID da Ordem: {oid[:28]}…",
                            f"token_id: {token_id}",
                            f"Mercado: {mt}",
                            f"Direção: {oc or '—'}",
                            f"Compra/Venda: {side} Alvo: {np} Volume: {sz_snap}",
                            f"Status: Cancelamento OK, mas falhou ao postar nova ordem (tentativa {attempt}) - Repetindo...",
                            "",
                            hint,
                            "",
                        ...
                        telegram.notify_operational_warning_zh(
                            title_zh="Falha ao repostar ordem após ajuste",
                            lines=lines,
                    ...
                    if apply_result.outcome == "replaced_ok":
                        ...
                        if telegram.enabled:
                            ...
                            title = (
                                "Falha ao repostar após ajuste (tentativas esgotadas)"
                                if apply_result.outcome == "replace_failed"
                                else "Falha ao cancelar para ajuste"
                            )
                            ...
                            lines = [
                                ...
                                f"Direção: {oc or '—'}",
                                f"Compra/Venda: {side}",
                                f"Preço Alvo: {apply_result.new_price}",
                                f"Volume: {apply_result.size}",
                                f"Razão do Ajuste: {apply_result.decision_reason}",
                                "",
                                hint,
                                "",
                                "Resposta da API:",
                        ...
                        if telegram.enabled:
                            ...
                            lines = [
                                ...
                                f"Direção: {oc or '—'}",
                                f"Compra/Venda: {side}",
                                f"Razão do Cancelamento: {apply_result.decision_reason}",
                                "",
                                hint,
                                "",
                                "Resposta da API:",
                            ...
                            telegram.notify_operational_warning_zh(
                                title_zh="【AVISO】Falha ao cancelar ordem",
                                lines=lines,
                        ...
                        if telegram.enabled:
                            ...
                            labels = {
                                "noop_missing_id": "Ordem sem ID, ignorando",
                                "noop_unknown_action": "Tipo de ação desconhecido, ignorando",
                                "replace_skip_bad_order": "Ajuste ignorado (ordem sem token ou lado)",
                                "replace_skip_size": "Ajuste ignorado (volume restante é 0)",
                            }
                            telegram.notify_operational_warning_zh(
                                title_zh=f"【AVISO】{labels.get(apply_result.outcome, apply_result.outcome)}",
                                lines=[
                                    f"ID da Ordem: {(oid or '')[:28]}…",
                                    f"Mercado: {mt}",
                                    f"Direção: {oc or '—'}",
                                    f"Resultado: {apply_result.outcome}",
                                ],
            ...
            if (
                ...
                for row in sorted(
                    ...
                    detail_lines.append(
                        f"· {title_short}\n"
                        f"  Direção:{outcome or '—'} {row['side']} Preço={row['price']:.4f} Mid={row['mid']:.4f}\n"
                        f"  |Preço−mid|/δ={ratio:.1%} (Razão {ratio:.3f}) Pontuação:{st} Ordem={str(row['oid'])[:18]}…"
                        f"{depth_block}"
                    )
                if not detail_lines and band_summary_eligible_n > 0:
                    detail_lines = [
                        "(Ordens elegíveis presentes, mas falha ao obter o preço médio (mid) nesta rodada)"
                    ]
                elif not detail_lines and orders:
                    detail_lines = [
                        "(Ordens abertas presentes, mas nenhuma elegível: fora da whitelist ou posição já existente)"
                    ]
                elif not detail_lines:
                    detail_lines = ["(Nenhuma ordem aberta no momento)"]
                bucket = int(now // iv)
                time_label = time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(now)
                )
